...
游戏行业的公司在发行游戏产品时，需要制作大量的广告视频素材用于投放。由于广告素材的生命周期通常很短，只有几天到几周时间，游戏公司为了保证广告投放效果稳定，必须持续制作大量视频素材进行AB测试。

**规模数据：**
- **存量素材库**：部分游戏公司的广告视频素材量已超过**10万+**级别
- **月新增量**：目前每月以**上万条**的速度增加，部分公司预计未来月新增视频超过**10万+**
- **AB测试需求**：为了做出新的爆款广告素材，需要大量的试错和迭代，这是广告视频数量爆发式增长的主要原因

**具体挑战：**
- **海量素材库检索难题**：面对**10万+级别**的广告视频库，传统的文件名和标签检索方式已经无法满足需求
- **创意检索效率低下**：在生成新广告视频流程中，设计师需要频繁从庞大的视频素材库中检索、提取素材信息和片段，获取创意灵感或拼接新的广告视频
- **AB测试素材需求爆发**：为了满足广告素材的AB测试，需要快速找到相似风格、相似元素的历史素材作为参考
- **跨模态搜索需求**：需要通过文本描述、参考图片或音频片段快速找到相关的视频素材
- **团队协作效率**：减少美术、策划和制作团队在素材查找和筛选上的时间成本

### 其他行业应用场景

**广告行业（效果类广告）：**
- 效果类广告与游戏广告具有相似的特点：都需要大量的AB测试、快速迭代和素材复用。因此，游戏行业的多模态搜索方案可以直接复用到效果类广告领域：
