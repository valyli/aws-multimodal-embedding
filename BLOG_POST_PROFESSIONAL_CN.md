...
文本到音频的跨模态搜索能力：
- **语义音频匹配**：使用文本描述找到相关音频内容
- **音频播放器集成**：搜索结果中的直接播放控件
- **相似度评分**：量化文本到音频内容匹配的准确性
- **多样化结果**：支持音乐、语音、环境声音等各类音频内容
...
搜索结果界面提供了丰富的信息和交互功能：
- **相似度分数**：量化匹配置信度，帮助用户理解结果相关性
- **媒体预览**：视频和音频的直接播放，图像的即时显示
- **元数据信息**：文件类型、上传时间和其他详细信息
- **排序机制**：按相关性排序的结果，确保最佳匹配优先显示
- **跨模态标识**：清晰标示搜索类型和匹配的embedding类型
